import { defineComponent, ref, computed, mergeProps, unref, useSSRContext } from 'file://C:/Project/Van%20Elvina/van-elvina/node_modules/vue/index.mjs';
import { ssrRenderAttrs, ssrRenderList, ssrRenderClass, ssrInterpolate, ssrRenderAttr, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual } from 'file://C:/Project/Van%20Elvina/van-elvina/node_modules/vue/server-renderer/index.mjs';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(activeTab) === "banners") {
        _push(`<div class="space-y-6"><div class="flex items-center justify-between"><h2 class="text-xl font-serif text-deep-plum font-semibold">Manage Hero Banners</h2><button class="px-4 py-2 bg-deep-plum text-white hover:bg-plum-800 rounded-lg text-xs font-semibold shadow-sm"> + Add Slide </button></div><div class="border border-rose-blush/20 rounded-xl overflow-hidden"><table class="w-full text-left border-collapse text-xs"><thead><tr class="bg-warm-ivory text-deep-plum border-b border-rose-blush/20 font-semibold"><th class="p-3">Preview</th><th class="p-3">Badge &amp; Title</th><th class="p-3">CTA Link</th><th class="p-3 text-right">Actions</th></tr></thead><tbody class="divide-y divide-rose-blush/10"><!--[-->`);
        ssrRenderList(unref(banners), (banner) => {
          _push(`<tr class="hover:bg-warm-ivory/20"><td class="p-3"><img${ssrRenderAttr("src", banner.image)} class="w-20 h-10 object-cover rounded shadow-sm bg-warm-ivory"></td><td class="p-3 font-ui">`);
          if (banner.badge) {
            _push(`<span class="px-2 py-0.5 bg-dusty-rose/10 text-dusty-rose rounded text-[10px] uppercase font-bold">${ssrInterpolate(banner.badge)}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<p class="font-semibold text-charcoal mt-1">${ssrInterpolate(banner.title)}</p></td><td class="p-3 text-charcoal/60">${ssrInterpolate(banner.ctaLink)}</td><td class="p-3 text-right space-x-1.5"><button class="text-deep-plum hover:underline font-semibold">Edit</button><button class="text-red-500 hover:underline font-semibold">Delete</button></td></tr>`);
        });
      ...
      if (unref(bannerModal).show) {
        _push(`<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm"><div class="bg-white rounded-2xl w-full max-w-lg p-6 overflow-y-auto max-h-[90vh] border border-rose-blush/20 shadow-xl space-y-4"><h3 class="text-lg font-serif text-deep-plum font-bold">${ssrInterpolate(unref(bannerModal).isEdit ? "Edit Banner Slide" : "Add Banner Slide")}</h3><form class="space-y-4 text-xs font-ui"><div><label class="block font-semibold mb-1 text-charcoal/70">Badge Text (e.g. New Collection)</label><input${ssrRenderAttr("value", unref(bannerModal).form.badge)} type="text" class="w-full p-2 border border-rose-blush/30 rounded"></div><div><label class="block font-semibold mb-1 text-charcoal/70">Main Title *</label><input${ssrRenderAttr("value", unref(bannerModal).form.title)} type="text" required class="w-full p-2 border border-rose-blush/30 rounded"></div><div><label class="block font-semibold mb-1 text-charcoal/70">Subtitle (Carousel Description) *</label><textarea rows="3" required class="w-full p-2 border border-rose-blush/30 rounded">${ssrInterpolate(unref(bannerModal).form.subtitle)}</textarea></div><div><label class="block font-semibold mb-1 text-charcoal/70">CTA Button Text (e.g. Shop Now) *</label><input${ssrRenderAttr("value", unref(bannerModal).form.cta)} type="text" required class="w-full p-2 border border-rose-blush/30 rounded"></div><div><label class="block font-semibold mb-1 text-charcoal/70">CTA Link *</label><input${ssrRenderAttr("value", unref(bannerModal).form.ctaLink)} type="text" required class="w-full p-2 border border-rose-blush/30 rounded"></div><div><label class="block font-semibold mb-1 text-charcoal/70">Image (Upload direct file or enter URL) *</label><div class="flex gap-2"><input${ssrRenderAttr("value", unref(bannerModal).form.image)} type="text" placeholder="http://..." required class="flex-1 p-2 border border-rose-blush/30 rounded"><input type="file" class="hidden" id="banner-file"><label for="banner-file" class="px-3 py-2 bg-rose-blush text-deep-plum font-semibold rounded cursor-pointer hover:bg-rose-blush/80">Upload File</label></div><p class="text-[10px] text-charcoal/40 mt-1">Image size tip: 1400x600 pixels is best.</p></div><div><label class="block font-semibold mb-1 text-charcoal/70">Text Color *</label><select class="w-full p-2 border border-rose-blush/30 rounded"><option value="light"${ssrIncludeBooleanAttr(Array.isArray(unref(bannerModal).form.textColor) ? ssrLooseContain(unref(bannerModal).form.textColor, "light") : ssrLooseEqual(unref(bannerModal).form.textColor, "light")) ? " selected" : ""}>Light Text (For dark backgrounds)</option><option value="dark"${ssrIncludeBooleanAttr(Array.isArray(unref(bannerModal).form.textColor) ? ssrLooseContain(unref(bannerModal).form.textColor, "dark") : ssrLooseEqual(unref(bannerModal).form.textColor, "dark")) ? " selected" : ""}>Dark Text (For light backgrounds)</option></select></div><div><label class="block font-semibold mb-1 text-charcoal/70">Alignment *</label><select class="w-full p-2 border border-rose-blush/30 rounded"><option value="left"${ssrIncludeBooleanAttr(Array.isArray(unref(bannerModal).form.align) ? ssrLooseContain(unref(bannerModal).form.align, "left") : ssrLooseEqual(unref(bannerModal).form.align, "left")) ? " selected" : ""}>Left</option><option value="center"${ssrIncludeBooleanAttr(Array.isArray(unref(bannerModal).form.align) ? ssrLooseContain(unref(bannerModal).form.align, "center") : ssrLooseEqual(unref(bannerModal).form.align, "center")) ? " selected" : ""}>Center</option><option value="right"${ssrIncludeBooleanAttr(Array.isArray(unref(bannerModal).form.align) ? ssrLooseContain(unref(bannerModal).form.align, "right") : ssrLooseEqual(unref(bannerModal).form.align, "right")) ? " selected" : ""}>Right</option></select></div><div class="flex justify-end gap-2 pt-2"><button type="button" class="px-4 py-2 border border-rose-blush/40 rounded text-charcoal">Cancel</button><button type="submit" class="px-5 py-2 bg-deep-plum text-white rounded">Save</button></div></form></div></div>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(categoryModal).show) {
        _push(`<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm"><div class="bg-white rounded-2xl w-full max-w-lg p-6 overflow-y-auto max-h-[90vh] border border-rose-blush/20 shadow-xl space-y-4"><h3 class="text-lg font-serif text-deep-plum font-bold">${ssrInterpolate(unref(categoryModal).isEdit ? "Edit Category" : "Create Category")}</h3><form class="space-y-4 text-xs font-ui"><div><label class="block font-semibold mb-1 text-charcoal/70">Category Name *</label><input${ssrRenderAttr("value", unref(categoryModal).form.name)} type="text" required class="w-full p-2 border border-rose-blush/30 rounded"></div><div><label class="block font-semibold mb-1 text-charcoal/70">Category Slug *</label><input${ssrRenderAttr("value", unref(categoryModal).form.slug)} type="text" required class="w-full p-2 border border-rose-blush/30 rounded" placeholder="bras"></div><div><label class="block font-semibold mb-1 text-charcoal/70">Description</label><textarea rows="2" class="w-full p-2 border border-rose-blush/30 rounded">${ssrInterpolate(unref(categoryModal).form.description)}</textarea></div><div><label class="block font-semibold mb-1 text-charcoal/70">Cover Image URL *</label><div class="flex gap-2"><input${ssrRenderAttr("value", unref(categoryModal).form.image)} type="text" required class="flex-1 p-2 border border-rose-blush/30 rounded" placeholder="http://..."><input type="file" class="hidden" id="category-file"><label for="category-file" class="px-3 py-2 bg-rose-blush text-deep-plum font-semibold rounded cursor-pointer hover:bg-rose-blush/80">Upload File</label></div></div><div><div class="flex items-center justify-between mb-1"><label class="block font-semibold text-charcoal/70">Subcategories</label><button type="button" class="text-deep-plum font-semibold hover:underline">+ Add subcategory</button></div><div class="space-y-2"><!--[-->`);
      ...
      if (unref(productModal).show) {
        ...
        ssrRenderList(unref(productModal).form.variants, (v, vIdx) => {
          ...
          ssrRenderList(v.images, (img, imgIdx) => {
            _push(`<div class="flex gap-2"><input${ssrRenderAttr("value", v.images[imgIdx])} placeholder="http://..." class="flex-1 p-2 border border-rose-blush/30 rounded"><input type="file" class="hidden"${ssrRenderAttr("id", `prod-file-${vIdx}-${imgIdx}`)}><label${ssrRenderAttr("for", `prod-file-${vIdx}-${imgIdx}`)} class="px-2.5 py-2 bg-rose-blush text-deep-plum font-semibold rounded cursor-pointer">Upload</label><button type="button" class="text-red-500">Remove</button></div>`);
          });
          _push(`<!--]--></div></div></div>`);
        });
      ...
      if (unref(widgetModal).show) {
        ...
        if (unref(widgetModal).form.type !== "html") {
          _push(`<div><label class="block font-semibold mb-1 text-charcoal/70">Section Image (Banner / Editorial Image)</label><div class="flex gap-2"><input${ssrRenderAttr("value", unref(widgetModal).form.image)} type="text" class="flex-1 p-2 border border-rose-blush/30 rounded" placeholder="http://..."><input type="file" class="hidden" id="widget-file"><label for="widget-file" class="px-3 py-2 bg-rose-blush text-deep-plum font-semibold rounded cursor-pointer">Upload</label></div></div>`);
        } else {
          _push(`<!---->`);
        }
...
//# sourceMappingURL=dashboard-DvirsIU6.mjs.map
