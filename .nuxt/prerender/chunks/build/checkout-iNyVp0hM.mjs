import { defineComponent, ref, reactive, computed, mergeProps, unref, withCtx, createTextVNode, isRef, toDisplayString, useSSRContext } from 'file://C:/Project/Van%20Elvina/van-elvina/node_modules/vue/index.mjs';
import { ssrRenderAttrs, ssrRenderList, ssrRenderClass, ssrInterpolate, ssrRenderStyle, ssrRenderComponent, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderAttr, ssrRenderSlot } from 'file://C:/Project/Van%20Elvina/van-elvina/node_modules/vue/server-renderer/index.mjs';
import { _ as _sfc_main$2 } from './AppButton-f0i-AWLW.mjs';
import { f as formatPrice, g as generateOrderId } from './formatters-CRqlhyd1.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const paymentMethods = [
      { id: "upi", icon: "\u{1F4B3}", name: "UPI", desc: "Google Pay, PhonePe, Paytm, BHIM" },
      { id: "card", icon: "\u{1F3E6}", name: "Credit / Debit Card", desc: "Visa, Mastercard, RuPay, Amex" },
      { id: "netbanking", icon: "\u{1F310}", name: "Net Banking", desc: "All major Indian banks" },
      { id: "cod", icon: "\u{1F4B5}", name: "Cash on Delivery", desc: "Available in select PIN codes" }
    ];
    const shippingFee = computed(() => {
      var _a, _b;
      return (_b = (_a = shippingOptions.find((o) => o.id === selectedShipping.value)) == null ? void 0 : _a.price) != null ? _b : 0;
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(["\u{1F512} SSL Secured", "\u2705 PCI Compliant", "\u{1F3E6} 100% Safe"], (b) => {
      ...
      ssrRenderList(unref(cart).items, (item) => {
        var _a;
        _push(`<div class="flex gap-3"><img${ssrRenderAttr("src", (_a = item.product.variants[0]) == null ? void 0 : _a.images[0])}${ssrRenderAttr("alt", item.product.name)} class="w-12 h-14 object-cover rounded-lg shrink-0" width="48" height="56"><div class="min-w-0"><p class="text-xs font-ui font-medium text-charcoal line-clamp-2">${ssrInterpolate(item.product.name)}</p><p class="text-[10px] text-mid-gray font-ui">${ssrInterpolate(item.variantColor)} \xB7 ${ssrInterpolate(item.size)} \xB7 Qty ${ssrInterpolate(item.quantity)}</p><p class="text-xs font-semibold text-charcoal font-ui">${ssrInterpolate(unref(formatPrice)(item.product.price * item.quantity))}</p></div></div>`);
      });
...
//# sourceMappingURL=checkout-iNyVp0hM.mjs.map
